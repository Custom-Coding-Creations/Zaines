...
import {
  parsePlayGroupAuditContent,
  PLAY_GROUP_AUDIT_PREFIX,
} from '@/lib/api/play-group-audit';
...
export async function GET(request: NextRequest) {
  ...
  const rows = await prisma.message.findMany({
    where: {
      content: {
        startsWith: PLAY_GROUP_AUDIT_PREFIX,
      },
    },
    orderBy: {
      sentAt: 'desc',
  ...
  const events = rows
    .map((row) => {
      const payload = parsePlayGroupAuditContent(row.content);
